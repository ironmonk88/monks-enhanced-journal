...
export class APSJ {
	...
	static blockList = ["black", "blue", "cyan", "green", "orange", "purple", "red", "yellow", "card", "scroll", "encounter", "read-aloud"];
	static dialogList = ["blue", "cyan", "green", "orange", "purple", "red", "yellow"];
	static panelList = ["bonus", "effect", "info", "loot", "note", "trap", "warning", "blue", "cyan", "green", "orange", "purple", "red", "yellow"];

	static async init() {
		APSJ.setTheme(setting("background-colour"));

		CONFIG.TinyMCE.content_css.push('modules/monks-enhanced-journal/css/apsjournal.css');
		...
		CONFIG.TinyMCE.templates = (CONFIG.TinyMCE.templates || []).concat(await Promise.all(APSJ.blockList.map(async (c) => {
			return {
 ...
 **/
	...
	static async getBlock(colour) {
		if (["card", "scroll", "encounter", "readAloud"].includes(colour)) {
			let content = await renderTemplate(`modules/monks-enhanced-journal/templates/apsjournal/${colour}.html`);
			return content;
		} else {
			let data = {
				colour: colour,
			...
			let content = await renderTemplate("modules/monks-enhanced-journal/templates/apsjournal/block.html", data);
			return content;
		}
	}

	static async getDialog(colour, side) {
		let content = await renderTemplate("modules/monks-enhanced-journal/templates/apsjournal/block.html", { colour, side });
		return content;
	}
	...
	static async getPanel(colour) {
		let data = {
			...
			icon: ["bonus", "effect", "information", "loot", "note", "trap", "warning"].includes(colour)
		};

		switch (colour) {
			case "bonus": data.colour = "cyan"; break;
			case "effect": data.colour = "purple"; break;
			case "information": data.colour = "blue"; break;
			case "loot": data.colour = "green"; break;
			case "note": data.colour = "yellow"; break;
			case "trap": data.colour = "orange"; break;
			case "warning": data.colour = "red"; break;
			default: data.colour = colour; break;
        }

		let content = await renderTemplate("modules/monks-enhanced-journal/templates/apsjournal/panel.html", data);
		return content;
    }
}
